...
export function CreateUserPage({ currentUser, onNavigate, onLogout }: CreateUserPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authenticatedFetch('http://localhost:5000/users/invite', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-[var(--space-xl)]">
      ...
      {/* Error Message */}
      {error && (
        <Card className="bg-[#FEF2F2] border-[var(--color-error)]">
          <div className="flex items-center gap-[var(--space-md)]">
            <div className="w-6 h-6 rounded-full bg-[var(--color-error)] text-white flex items-center justify-center font-bold">!</div>
            <div>
              <p className="font-bold text-[#991B1B]">Error Creating User</p>
              <p className="text-[var(--font-size-small)] text-[#B91C1C] mt-1">
                {error}
              </p>
            </div>
          </div>
