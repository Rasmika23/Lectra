...
export function ModuleManagementPage({ currentUser, onNavigate }: ModuleManagementPageProps) {
  ...
  useEffect(() => {
    ...
    const fetchLecturers = async () => {
      try {
        const response = await fetch(`http://localhost:5000/modules/${selectedModuleId}/lecturers`);
        if (response.ok) {
          const data = await response.json();
            ...
            <div className="mt-[var(--space-lg)] p-[var(--space-lg)] bg-[var(--color-bg-main)] rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-[var(--space-md)] text-[var(--font-size-small)]">
                <div>
                  ...
                  <span className="ml-2 font-medium">{selectedModule.academicYear}</span>
                </div>
                <div>
                  <span className="text-[var(--color-text-secondary)]">Semester:</span>
                  <span className="ml-2 font-medium">{selectedModule.semester}</span>
                </div>
                <div className="col-span-2">
                  <span className="text-[var(--color-text-secondary)]">Sub-Coordinator:</span>
                  <span className="ml-2 font-medium">{selectedModule.subCoordinator || 'Not assigned'}</span>
                </div>
              </div>
            </div>
            ...
            {moduleLecturers.length > 0 ? (
              <div className="mt-[var(--space-lg)] p-[var(--space-lg)] bg-[var(--color-bg-main)] rounded-lg">
                <h3 className="font-bold text-[var(--color-text-primary)] mb-[var(--space-md)]">
                  Assigned Lecturers ({moduleLecturers.length})
                </h3>
                  ...
                  {moduleLecturers.map((lecturer) => (
                    ...
                    >
                      <div className="flex items-center gap-[var(--space-md)]">
                        ...
                        <span className="font-medium text-[var(--color-text-primary)]">{lecturer.name}</span>
                      </div>
                      <span className="text-sm text-gray-400">{lecturer.email}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-[var(--space-lg)] p-[var(--space-lg)] bg-[var(--color-bg-main)] rounded-lg">
                <p className="text-gray-500 italic">No lecturers assigned to this module.</p>
              </div>
            )}
          </>
        )}
